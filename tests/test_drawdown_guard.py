...
def test_tiers():
    ...
    assert g.evaluate(8.0)["mode"] == "defensive"
    assert g.evaluate(12.0)["mode"] == "block_new_trades"
    assert g.evaluate(18.0)["mode"] == "emergency_flatten"


def test_risk_multiplier():
    g = DrawdownGuard()
    assert g.evaluate(5.0)["risk_multiplier"] == 1.0
    assert g.evaluate(9.0)["risk_multiplier"] == 0.5
    assert g.evaluate(13.0)["risk_multiplier"] == 0.0
